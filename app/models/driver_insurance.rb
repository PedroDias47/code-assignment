...
#   multiplied by the daily price which is recorded in the associated vehicle (varying by vehicle)
...
class DriverInsurance < ActiveRecord::Base
  ...
  def numds_between_dates(beginning_date, ending_date)
    return 0 if start_date > ending_date || end_date < beginning_date

    begin_date = start_date < beginning_date ? beginning_date : start_date
    finish_date = end_date > ending_date ? ending_date : end_date

    (finish_date - begin_date).to_f
  end

  def driver_insurance_price
    (end_date - start_date).to_f * vehicle.driver_insurance_daily_rate_pounds    
  end
end
